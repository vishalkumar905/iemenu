                                ...
                                <div class="card">
                                    <div class="card-image" data-header-animation="true">
                                      <a href="<?= base_url('Menu_item/index').'/'.$menus->menu_id;?>">
                                        <img class="img menu_image" src="../../<?= ($menus->image) ? $menus->image : 'assets/img/image_placeholder.jpg' ?>" style="max-height: 156.75px;min-height: 156.75px;">
                                      </a>
                                      <style>
                                        .menu_image { max-height: 135px; min-height: 130px; }
                                      </style>
                                    </div>
                                    <div class="card-content">
                                        <div class="card-actions">
                                            ...
                                            <button type="button" class="btn btn-danger btn-simple" rel="tooltip" data-placement="bottom" title="Remove" onclick="delete_Menu('<?= ($menus->menu_id) ?>', '<?= $rest_id ?>')">
                                    ...
                                    <div class="card-footer">
                                        <div class="price">
                                            ...
                                            <div class="card-description"><?= substr($menus->description, 0, 30); ?></div>
                                        </div>
                                        <div class="stats pull-right">
                                            <div class="togglebutton">
                                              <label><input type="checkbox" id="publish" name="publish" <?php if($menus->is_publish=='Yes') { echo 'checked'; } ?> onclick="manage_status(this,'<?= ($menus->menu_id) ?>','<?= $rest_id ?>')"></label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="modal-content">
                                          ...
                                          <div class="modal-body">
                                            <form id="menuForm" class="form-horizontal" method="POST" action="<?= base_url('menu/store_menu/update') ?>" enctype="multipart/form-data">
                                              ...
                                              <button type="submit" class="btn btn-rose" id="btn-menuForm" value="create">Update Section</button>
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <form id="menuForm" class="form-horizontal" method="POST" action="<?= base_url('menu/store_menu') ?>" enctype="multipart/form-data">
                          ...
                          <button type="submit" class="btn btn-rose" id="btn-menuForm" value="create">Add Section</button>
                          <button type="button" class="btn btn-danger btn-simple" data-dismiss="modal">Close</button>
                        </form>
                      </div>
                      <div class="modal-footer">
...
function delete_Menu(menuId=0,restID=0) {
    if (confirm("Do you really want to delete section?")) {
       location.href="<?= base_url('Menu/delete_menu') ?>" +'/'+ menuId +'/'+ restID;
    }
    return false;
}

function manage_status(obj,menuId=0,restID=0) {
    if($(obj). is(":checked")){ 
        location.href="<?= base_url('Menu/manage_status') ?>" +'/'+ menuId +'/'+ restID +'/Yes';
    }
    else {
        location.href="<?= base_url('Menu/manage_status') ?>" +'/'+ menuId +'/'+ restID +'/No';
    }
    return false;
}
