...
<body>
    <section class="pt-25">
        <div class="container">
			<div class="heading"><h2 class="sub-heading">Checkout</h2></div>
			<div class="row">
				<div class="col-md-6">
				    <?php $ci=&get_instance(); if($this->session->userdata('CartList')) {
					...
					<div class="cart-total">
						<h6 class="mt-10 bg-lite-blue">
                            <b>Sub-Total</b>
                            <?php $subtotal = 0;
                            if($this->session->userdata('CartList')) {
                                $CartLists=json_decode($this->session->userdata('CartList'), true);
                                $subtotal=$ci->cartTotal($CartLists,'no');
                            }
                            ?>
							<p class="float-right"><b>₹<span class="total-price"><?= $subtotal ?></span></b></p>
                        </h6>
                        <?php $taxLists=$ci->getTaxList($rest_id); if(!empty($taxLists)) :
                            foreach($taxLists as $taxList): ?>
						<h6 class="mt-10 bg-lite-blue">
                            <b><?= $taxList->tax_type ?> &nbsp; (<?= $taxList->tax_percent ?>%) </b>
							<p class="float-right"><b>₹<span class="total-price"><?= $ci->cartTax($CartLists, $taxList->tax_percent) ?></span></b></p>
                        </h6>
				...
				<div class="col-md-6">
						...
						<div class="heading"><h2 class="sub-heading">Details</h2></div>
						<div class="row">
