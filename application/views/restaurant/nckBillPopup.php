...
<div class="modal-header">
	...
	<h5 class="modal-title" id="myModalLabel2"><b>NCK Bill For Order #<?= $order->order_id ?></b></h5>
</div>
	    ...
	    <div class="row">
			<div class="col-md-12">
			    <h5  class="modal-title">
                    <b>NCK Bill For Table Name </b><?= ($order->table_id) ? ($ci->getTableDetail($order->table_id)) ? $ci->getTableDetail($order->table_id)->table_name : '-' : '-'; ?>
                </h5>
			</div>
	    </div>
    </div>
 ...
 <div class="row">
	...
	<div class="text-right">
	   ...
	   <?php if($order->order_status != 4) { ?>
		    <button type="button" class="btn btn-primary btn-round" onclick="updateOrder('<?= $order->order_id ?>','4')">NCK BILL</button>
	   
	    
	    
	    
	    
	<?php } elseif($order->order_status == 4) { $path=base_url("Restaurant/printInvoice2/").$order->order_id; $path2=base_url("Restaurant/printInvoice3/").$order->order_id; ?>
